...
export async function lastUpdatedDate(preview) {
  ...
  try {
    ...
    // Check if items exist and if there's at least one item
    if (items && items.length > 0) {
      // Access the updatedAt of the first item
      const updatedAt = items[0].sys.updatedAt;
      return updatedAt;
    } else {
      // Handle case when no items are returned
      console.error("No case studies found.");
      return null;
    }
  } catch (error) {
    // Let the caller handle the error
    throw new Error("Error fetching case study collection: " + error.message);
  }
}
