...
export default function BlockFooter({ data }) {
  ...
  return (
    ...
    <motion.div  ref={footerRef}  className="relative flex flex-col justify-between overflow-clip h-vhh">
      {/* pt-32 pb-16 */}
      ...
      <div className="z-20 grid w-full grid-cols-12 gap-3 px-16 py-16">
        <div className="flex flex-col col-span-12 gap-12 md:col-span-7">
          ...
          {data?.title && (
            <motion.p className="text-sm" style={{color: 'var(--text-color-inv)'}}>{data?.title}</motion.p>
          )}
...
{data.content && (
      ...
      <div className="z-20 flex items-center justify-between w-full px-16 py-8">
        ...
        {data.devcredit && (
          <div className="!text-xs !font-monocode" style={{color:'var(--text-color)'}}>
            {documentToReactComponents(
              data.devcredit.json,
              RichTextOptions
            )}
