...
export default function Navigation() {
  ...
  <motion.div className={`logo_wrapper ${isActive ? "active" : ""} ${routeInfo.sourceRoute === '/' ? "text-white" :""}`}
  >
    <motion.span className="logo origin-top-left"
      onClick={ handleClick}
...
{/* //backdrop-blur-md  */}
<div className="flex space-x-2 absolute top-6 right-6 p-2 rounded-md" style={{zIndex:9999}}>
  
  {pages.map((page) => (
    <Link href={page.url} 
          key={page.id}
          onClick={() => setActivePage(page.id)}
          className={`relative rounded-md px-3 py-3 text-white ${activePage === page.id ? "text-black" : "hover:opacity-50 " }  rounded-full text-sm`}
          >
        <span className={` ${activePage === 'home' ? "" : "text-gray-900"} relative z-10 mix-blend-exclusion`}>
        {page.title}
        </span>
        
      {activePage === page.id && 
      <motion.div layoutId="indicator" className="absolute inset-0 top-0 bg-white rounded-md"></motion.div>
      }
