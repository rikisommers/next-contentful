export const perlinBlobVertexShader = `
...
void main() {
  ...
  vNoise = (2.0 * -uWaves) * turbulence(uDecay * abs(normal + uTime));
  vQnoise = (0.3 * -uEqcolor) * turbulence(uDecay * abs(normal + uTime));

  float b = pnoise(uComplex * position + vec3((uDecay * 2.0) * uTime), vec3(100.0));

  vDisplacement = -atan(vNoise) + tan(b * uDisplace);

  vec3 newPosition = position + (normal * vDisplacement);
  gl_Position = projectionMatrix * modelViewMatrix * vec4(newPosition, abs(uSize));
  gl_PointSize = 3.0;
}
`;